...
$date->modify('+' . ($endDate) . ' days');
$newTaskEft = $date->format('Y-m-d');
// show("Start: " . $startDate . " End: " . $endDate . " NewEftDate: " . $newTaskEft);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $projectStart = $_POST['projStartDate'];
    $pickedDate = new DateTime($projectStart);
    $diff = $date->diff($pickedDate);
    $newTaskStartDate = $diff->days + $endDate;
...
echo $newProjectName = $RawTasks[0]['Projectid'];
...
$firstTask = null;
function countAllSuccessors2($taskName, $Successors)
{
    ...
    foreach ($Successors[$taskName] as $successor) {
        $count += countAllSuccessors2($successor, $Successors);
    }
    return $count;
}
...
foreach ($TasksWithEstEft as $taskName => $task) {
    $numSuccessors = countAllSuccessors2($taskName, $Successors);
...
if (!is_null($firstTask)) {
    ...
    $TasksWithEstEft[$firstTask]["est"] = $newTaskStartDate;
    $TasksWithEstEft[$firstTask]["eft"] = $TasksWithEstEft[$firstTask]["est"] + $TasksWithEstEft[$firstTask]["duration"];
    ...
    foreach ($TasksWithEstEft as $taskName => $task) {
        ...
        if (empty($prerequisites) && $taskName !== $firstTask) {
            $TasksWithEstEft[$taskName]["est"] = $newTaskStartDate;
...
function findCriticalPath2($currentTask, $TasksWithEstEft, $criticalPath)
{
    global $firstTask;
    ...
    if ($maxPrerequisite !== null) {
        return findCriticalPath2($maxPrerequisite, $TasksWithEstEft, $criticalPath);
    }
...
$criticalPath = [];
$criticalPath = findCriticalPath2($lastTask, $TasksWithEstEft, $criticalPath);
...
foreach ($Modifiedtask as $task) {
    ...
    $sql = "INSERT INTO stask (Taskn, Description, Priority, Status, Due, Userid, Projectid, Prerequisite, EST, EFT, LST, LFT, CompanyN, Uc) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)";
