...
<body>
    ...
    <div class="main-content">
        ...
        if (isset($_POST["addmember"])) {
            ...
            $query = "SELECT * FROM sworkers WHERE Semail = '$email'";
            $result = mysqli_query($conn, $query);


            if (mysqli_num_rows($result) > 0) {
                array_push($errors, "<div class='error'>Email is already associated with an existing account.</div>");
            }
            if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                array_push($errors,"<div class='error'>Invalid email format</div>");
            }
            ...
            <span style='font-size: 24px; font-weight: bold; color: #76abae; font-family: Courier New;'>SchedSpace PMS</span>";
                ...
                if ($mail->send()) {
                    ...
                    if (mysqli_stmt_prepare($stmt, $sql)) {
                        ...
                        echo  array_push($errors,"<div class='success'>Employee account was successfully created.</div>");
                    } else {
                        die("Error: " . mysqli_error($conn));
                    }
                }
